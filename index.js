...
const defaultBehaviour = (auth, gmail, coredata) => {
  return askForFilter().then((option) => {
    if (option === "label") {
      ...
      return askForMail().then((mailId) => {
        spinner.start();
        return getListOfMailIdByFromId(auth, mailId, 50);
      });
    } else if (option === "filter") {
      return askForFilterValue().then((filter) => {
        spinner.start();
...
const scanForLabelOption = (auth, gmail) => {
  return new Promise((resolve, reject) => {
    ...
    return listLabels(auth, gmail).then((response) => {
...
function fetchAndSaveAttachment(auth, attachment) {
  return new Promise((resolve, reject) => {
    ...
    var fileName = path.resolve(__dirname, "files", "Bajaj", attachment.name);
