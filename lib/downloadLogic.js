...
function fetchAndSaveAttachment(auth, attachment, folderName) {
    return new Promise((resolve, reject) => {
        ...
        var fileName = path.join(folderName, attachment.name.replaceAll("/", "-"));
...
async function processCompany(company, auth, gmailInstance, startDate, endDate, downloadFolder) {
    gmail = gmailInstance;

    const folderPath = path.join(downloadFolder || './files', company.name);
    ...
    await fetchAndSaveAttachments(auth, coredata.attachments, folderPath);
